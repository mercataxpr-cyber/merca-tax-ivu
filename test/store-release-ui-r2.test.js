...
test('store release cleanup loads after vNext and copy normalization', () => {
  const vnext = build.indexOf('src/mobile-vnext-ui.js');
  const copy = build.indexOf('src/mobile-copy-r1.js');
  const release = build.indexOf('src/store-release-ui-r2.js');
...
test('store release cleanup removes only unreleased presentation surfaces', () => {
  ...
  assert.match(cleanup, /scope\.matches/);
